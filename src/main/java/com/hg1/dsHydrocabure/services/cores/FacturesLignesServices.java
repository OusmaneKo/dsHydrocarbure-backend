...
import com.hg1.dsHydrocabure.repositories.cores.FactureLigneRepository;
...
@Slf4j
@Service
public class FacturesLignesServices {
    private final FactureLigneRepository factureLigneRepository;

    public FacturesLignesServices(FactureLigneRepository factureLigneRepository) {
    ...
    private ResponseWrapper<FacturesLignes> updateOld(FacturesLignes oldfl, FacturesLignes fl) {
        try {
            factureLigneRepository.saveAndFlush(fl);
            return ResponseWrapper.ok(fl);
        } catch (Exception e) {
    ...
    public ResponseWrapper<FacturesLignes> update(Long id, FacturesLignes fl) {
        try {

            return factureLigneRepository
                    .findById(id)
                    .map(oldfl -> updateOld(oldfl, fl))
